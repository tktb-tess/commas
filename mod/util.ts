 ...
 * @param nums
 */
export function toBigInt(nums: number | number[]) {
...
export const millerRabin = (n_: bigint) => {
  if (typeof n_ !== 'bigint')
    throw TypeError('引数型は `bigint` でなければなりません');
...
export const getRandIntFromDate = async () => {
  ...
  const hashed = new Uint32Array(
    await crypto.subtle.digest('SHA-256', utf8arr),
    0,
    1
  );
  return hashed[0];
